...
class QuoteImages extends StatefulWidget {
  @override
  _QuoteImagesState createState() => _QuoteImagesState();
}

class _QuoteImagesState extends State<QuoteImages> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<QuerySnapshot>(
      future: fb.collection('QuoteImages').get(),
...
class ImageDetail extends StatefulWidget {
  ...
  const ImageDetail({Key key, @required this.imageUrl, this.initialPage})
      : super(key: key);
...
class _ImageDetailState extends State<ImageDetail> {
  ...
  downloadImage(String imageUrl) async {
    try {
      ...
      if (status.isGranted) {
        onLoading(true, 'saving'); // loading show
        var imageId = await ImageDownloader.downloadImage(imageUrl);
        onLoading(false, 'saving'); // loading close
        var path = await ImageDownloader.findPath(imageId);
        showToast('Image Downloaded : $path');

        if (imageId == null) {
          showToast('can not download image');
        }
  ...
  Future setWallpaper(BuildContext context, String imageUrl) async {
    Navigator.of(context, rootNavigator: true).pop();
    onLoading(true, 'waiting'); //loading show
    try {
      Future.delayed(
        Duration(seconds: 1),
        () async {
          ...
          onLoading(false, 'waiting'); //loading close
